...
//import getSaRecord from '@salesforce/apex/MobileMapLayersService.executeFilterQuery1';//sri
import Id from '@salesforce/user/Id';
import { config } from './config';
import ConfirmModal from 'c/confirmModal';
import overrideCSS from './overrideCSS';
//import { getRecord } from 'lightning/uiRecordApi';//sri
...
export default class MobileMapLayersMain extends NavigationMixin(LightningElement) {
  ...
  GetAssignedResourceLocation({ data, errors }) {
    //console.log("resource location ", data)
  ...
  async getObjectInfos({ data, error }) {
    if (data) {
      ...
      this.CONFIG.mapObjects.forEach(async (o, ind) => {
        ...
        //console.log('-objQueries - ', this.objQueries);
        ...
        if (this.CONFIG.mapObjects.every((obj) => obj.infoReceived)) {
          ...
          //console.log('------------query', this.fullQuery);
        }
      });
    } else if (error) {
      this.handleError(error);
  ...
  buildQueryForObject = (obj) => {
    //console.log('key 2 --> ', obj);
    const stdKey = ["2BCEL1","2BLOC1","2BLOR1","2BLOS1","2BLOT1","12BLPF1","2EXFR1","2MAIBL1","2MRBL1","2PDASR","12TBCE1","12TBLC1",
    "12TBLR1","12TBLS1","12TBLT1","12TEHP1","12TGUL1","12TMAI1","12TMAR1","12TPRF1","SSBLOS1","SSBLOT1","SSMRBL1","2OOHCB"];
    let q;
    if(obj.value == 'ServiceAppointment'){

      q = `${obj.value}: ${obj.value} (where: { and: [ { ${obj.stdTextKeyField}: { in: ["2BCEL1","2BLOC1","2BCEL1","2BLOC1","2BLOR1","2BLOS1","2BLOT1","12BLPF1","2EXFR1","2MAIBL1","2MRBL1","2PDASR","12TBCE1","12TBLC1","12TBLR1","12TBLS1","12TBLT1","12TEHP1","12TGUL1","12TMAI1","12TMAR1","12TPRF1","SSBLOS1","SSBLOT1","SSMRBL1","2OOHCB"]} }, { ${obj.latField}: { ne: null } }, { ${obj.longField}: { ne: null } }, { Status: { nin: ["Completed","Cannot Complete","Cancelled"] } },{ ${obj.dueDateField}: { eq: { literal: NEXT_WEEK }}} ]}, first: 500) @category(name: "recordQuery") {
        edges {
          node {
            Id
            ${obj.latField} {
      ...
      q = `${obj.value}: ${obj.value} (where: { and: [ { ${obj.latField}: { ne: null } }, { ${obj.longField}: { ne: null } } ]}, first: 500) @category(name: "recordQuery") {
        edges {
          node {
            Id
            ${obj.latField} {
  ...
  // connectedCallback(){console.log('record id : ',this.recordId);}

  // @wire(getSaRecord)
  // //wiredServiceAppointement
  //  wiredServiceAppointement({data,errors}){
  //   if(data){
  //     console.log("getSaRecord@@",data)
  //     alert(JSON.stringify(data),'alert 1-------------')
  // ---
  //   if (data) {
